...
function isMonoEnabled() {
  ...
  Lampa.Lang.add({
    ...
    interface_mod_new_colored_ratings_desc: {
      ...
      uk: 'Включить цветное выделение рейтинга в полной карточке'
    },    
    interface_mod_new_show_status: {
      en: 'Show statuses',
      uk: 'Показывать статусы'
    },
    interface_mod_new_show_status_desc: {
      en: 'Show or hide movie/series statuses',
      uk: 'Отображать или скрывать статус фильма/сериала (например, "Выпущено")'
    },
    interface_mod_new_show_age: {
      en: 'Show age rating',
      uk: 'Показывать возрастной рейтинг'
    },
    interface_mod_new_show_age_desc: {
      en: 'Show or hide age rating badge (PG)',
      uk: 'Отображать или скрывать плашку возрастного рейтинга (PG)'
    },    
    ...
    interface_mod_new_colored_status_desc: {
      ...
      uk: 'Подсвечивать статус фильма/сериала в полной карточке'
    },
    interface_mod_new_colored_age: {
      en: 'Colored age rating',
      uk: 'Цветной возрастной рейтинг '
    },
    interface_mod_new_colored_age_desc: {
      en: 'Colorize age rating',
      uk: 'Подсвечивать возрастной рейтинг в полной карточке'
    },
    interface_mod_new_mono_mode: {
      en: 'Monochrome override',
      uk: 'Монохромный режим (Ч/Б)'
    },
    interface_mod_new_mono_mode_desc: {
      ...
      uk: 'Переопределяет цвета для статусов, PG и новой инфо-панели (если соответствующие опции включены)'
    },   
    ...
    interface_mod_new_title_mode: {
      ...
      uk: 'Названия под заголовком'
    },
        
    interface_mod_new_title_mode_desc: {
      en: 'Show original title, localized title, both, or hide',
      uk: 'Показывать оригинальное, локализованное, оба или отключить'
    },
    ...
    interface_mod_new_tmdb_api_key_desc: { 
      ...
      uk: 'Свой ключ для точного получения русских названий' 
    },

   
    interface_mod_new_title_mode_off:  { en: 'No', uk: 'Нет' },
    interface_mod_new_title_mode_orig: { en: 'Original title', uk: 'Оригинальное название' },
    interface_mod_new_title_mode_loc:  { en: 'Localized title', uk: 'Локализованное название' },
    interface_mod_new_title_mode_orig_loc: { en: 'Original / Localized', uk: 'Оригинальное / Локализованное название' },
    
    interface_mod_new_title_size_name: { en: 'Title size', uk: 'Размер названия' },
    interface_mod_new_title_size_desc: { en: 'Font size (default 0.75)', uk: 'Размер шрифта (по умолч. 0.75)' },

    interface_mod_new_all_buttons_v1: {
      en: 'All buttons in card',
      uk: 'Все кнопки в карточке'
    },
    interface_mod_new_all_buttons_desc: {
      en: 'Show all buttons in the card.',
      uk: 'Показывает все кнопки в карточке (нужна перезагрузка)'
    },
    interface_mod_new_icon_only: {
      en: 'Icons only',
      uk: 'Кнопки без текста'
    },
    interface_mod_new_icon_only_desc: {
      en: 'Hide button labels, keep only icons',
      uk: 'Скрывает подписи на кнопках, оставляет только иконки'
    },
    ...
    interface_mod_new_colored_buttons_desc: {
      ...
      uk: 'Обновляет иконки и цвета кнопок онлайн, торренты, трейлеры'
    },

    torr_mod_frame: {
      uk: 'Цветная рамка блока торрентов',
      en: 'Colored torrent frame by seeders'
    },
    torr_mod_frame_desc: {
      uk: 'Подсвечивать блоки торрентов цветной рамкой в зависимости от количества сидеров',
    ...
    torr_mod_bitrate_desc: {
      uk: 'Подсвечивать битрейт цветом в зависимости от размера',
      en: 'Color bitrate by value'
    },
    torr_mod_seeds: {
      uk: 'Цветное количество раздающих',
      en: 'Seeder count coloring'
    },
    torr_mod_seeds_desc: {
      uk: 'Подсвечивать количество сидеров на раздаче: \n0–5 — красный, 6–19 — жёлтый, 20 и выше — зелёный',
...
function applyMargins() {
  ...
  // Если пользователь ввёл просто число (например -1), добавляем em
...
function buildInfoPanel(details, movie, isTvShow, originalDetails) {
  ...
  var colors = {
    ...
    genres: {
      ...
      'Боевик и приключения': { bg: 'rgba(231,76,60,.85)', text: 'white' }, 'Детский': { bg: 'rgba(0,188,212,.85)', text: 'white' },
      'Новости': { bg: 'rgba(70,130,180,.85)', text: 'white' }, 'Реалити-шоу': { bg: 'rgba(230,126,34,.9)', text: 'white' },
      'НФ и фэнтези': { bg: 'rgba(41,128,185,.85)', text: 'white' }, 'Мыльная опера': { bg: 'rgba(233,30,99,.85)', text: 'white' },
      'Ток-шоу': { bg: 'rgba(241,196,15,.9)', text: 'black' }, 'Война и политика': { bg: 'rgba(96,125,139,.85)', text: 'white' }
    }
  };

  function ruGenreName(name) {
    return name;
  }
  ...
  if (isTvShow && seasonsCount > 0) {
    row4.append(
      $('<span>').text('Сезоны: ' + seasonsCount).css(badgeCss(colors.seasons.bg, colors.seasons.text))
    );
  }

  var genreList = [];
  if (Array.isArray(movie.genres) && movie.genres.length) {
    genreList = movie.genres.map(function (g) { return ruGenreName(g.name); });
  }
...
function applyStatusOnceIn(elRoot) {
  ...
  $root.find(STATUS_BASE_SEL).each(function () {
    ...
    if (/заверш|законч|ended/i.test(t)) key = 'completed';
    else if (/отмен|cancel(l)?ed/i.test(t)) key = 'canceled';
    else if (/продолж|возвращ|returning/i.test(t)) key = 'ongoing';
    else if (/производств|in\s*production/i.test(t)) key = 'production';
    else if (/заплан|planned/i.test(t)) key = 'planned';
    else if (/пилот|pilot/i.test(t)) key = 'pilot';
    else if (/выпущ|released/i.test(t)) key = 'released';
    else if (/слух|rumored/i.test(t)) key = 'rumored';
    else if (/пост/i.test(t) || /post/i.test(t)) key = 'post';
    else if (/скоро|soon/i.test(t)) key = 'soon';
    ...
    if (key && statusRu[key] && t !== statusRu[key]) $(el).text(statusRu[key]);
...
function applyOriginalTitleToggle() {
  ...
  function isPlayBtn($b) {
    ...
    if (/^(play|старт|воспроизвести|продолжить)$/i.test(txt)) return true;
    return false;
  }
...
(function(){
  Lampa.Lang.add({
    ifx_year_on_cards:         { uk:'Показывать год на карточке', en:'Show year on card' },
    ifx_year_on_cards_desc:    { uk:'Включить/выключить отображение года на постере', en:'Year displayed on the poster only' },

    ifx_show_rating_on_cards:      { uk:'Показывать рейтинг на карточке', en:'Show rating on card' },
    ifx_show_rating_on_cards_desc: { uk:'Включить/выключить стандартный рейтинг на постере',
                                   en:'Toggle the built-in rating badge on list posters' },
      
    ifx_alt_badges:      { uk:'Альтернативные метки', en:'Alternative badges' },
    ifx_alt_badges_desc: { uk:'Метки "год" и "рейтинг" в другом стиле', en:'Year & Rating  alternative style' },

    ifx_type_badges:           { uk:'Показывать метки "Фильм / Сериал" снизу', en:'"Show Movie / Series" badges (bottom)' },
    ifx_type_badges_desc:      { uk:'Заменяет стандартную метку TV на новые цветные метки внизу постера.', en:'Replaces the default TV badge with new colored badges at the bottom.' },

    ifx_alt_type_badges:       { uk:'Добавить дополнительную метку "Фильм" сверху', en:'Add "Movie" badge (top)' },
    ifx_alt_type_badges_desc:  { uk:'Добавляет синюю метку "Movie" на постер, которая дополняет стандартную красную метку "TV".', en:'Adds a blue "Movie" badge matching the default red "TV" badge.' },
    
    ifx_episode_alt_cards:     { uk:'Альтернативные "Ближайшие эпизоды"', en:'Alternative "Upcoming Episodes"' },
    ifx_episode_alt_cards_desc:{ uk:'Компактный вид блока "Ближайшие эпизоды"', en:'Compact view for the "Upcoming Episodes" block' },

    ifx_episode_num_only:      { uk:'Показывать только номер серии', en:'Show episode number only' },
...
function ensureTypeBadgesCss() {
  ...
  st.textContent = `
    ...
    body.ifx-alt-type-badges .card__type.ifx-movie-type {
        background: #4285F4 !important; /* Синий цвет */
        color: #fff !important;
    }
...
function ensureAltBadgesCss(){
  ...
  function episodeWord(){
    var code = (Lampa.Lang && Lampa.Lang.code) || 'ru';
    return code.indexOf('en')===0 ? 'Episode' : 'Серия';
  }
