(function() {
    ...
    // Альтернативный localhost для нерабочих источников
    // Раскомментируйте и укажите альтернативный хост
    var Defined_alt = {
        localhost: 'https://79.137.205.241:80/'
     };
    ...
    if (!window.rch) {
        Lampa.Utils.putScript([prox_prefix + Defined.localhost + 'invc-rch.js'], function() {}, false, function() {
            if (!window.rch.startTypeInvoke) window.rch.typeInvoke(prox_prefix + Defined.localhost, function() {});
        }, true);
    }
    ...
    function rchRun(json, call) {
        if (typeof signalR == 'undefined') {
            Lampa.Utils.putScript([prox_prefix + Defined.localhost + 'signalr-6.0.25_es5.js'], function() {}, false, function() {
                rchInvoke(json, call);
            }, true);
        } else {
            rchInvoke(json, call);
    ...
    function component(object) {
        ...
        if (balansers_with_search == undefined) {
            ...
            network.silent(prox_prefix + account(Defined.localhost + 'lite/withsearch'), function(json) {
        ...
        this.createSource = function() {
            ...
            return new Promise(function(resolve, reject) {
                ...
                // Запрос к основному хосту
                network.silent(prox_prefix + account(url), function(json) {
                    if (json.accsdb) return reject(json);
                    
                    // Если есть альтернативный хост - запрашиваем и его
                    if (typeof Defined_alt !== 'undefined') {
                        var url_alt = _this4.requestParams(Defined_alt.localhost + 'lite/events?life=true');
                        network.silent(prox_prefix + account(url_alt), function(json_alt) {
                            if (!json_alt.accsdb && json_alt && json_alt.online) {
                                ...
                                var mainSources = json.online || [];
                                var altSources = json_alt.online || [];
                                var mergedSources = [];
                                
                                // Создаем мапу источников с альтернативного хоста
                                var altMap = {};
                                altSources.forEach(function(s) {
                                    var name = balanserName(s);
                                    altMap[name] = s;
                                });
                                
                                // Объединяем: сначала все из альтернативного, потом недостающие из основного
                                altSources.forEach(function(s) {
                                    mergedSources.push(s);
                                });
                                mainSources.forEach(function(s) {
                                    var name = balanserName(s);
                                    if (!altMap[name]) {
                                        mergedSources.push(s);
                                    }
                                ...
                                json.online = mergedSources;
                            }
                            _this4.processSourceResponse(json, resolve, reject);
                        }, function() {
                            // Если альтернативный хост не отвечает - используем только основной
                            _this4.processSourceResponse(json, resolve, reject);
                        });
                    } else {
                        _this4.processSourceResponse(json, resolve, reject);
        ...
        this.processSourceResponse = function(json, resolve, reject) {
            var _this4 = this;
            if (json.life) {
        ...
        this.request = function(url) {
            ...
            if (number_of_requests < 10) {
                network["native"](prox_prefix + account(url), this.parse.bind(this), this.doesNotAnswer.bind(this), false, { dataType: 'text' });
        ...
        this.toPlayElement = function(file) {
            var play = { title: file.title, url: file.url, quality: file.qualitys, timeline: file.timeline, subtitles: file.subtitles, callback: file.mark };
            return play;
        };
            ...
            json.forEach(function(elem) {
                ...
                if (elem.img) {
                    ...
                    if (elem.img !== undefined) {
                        ...
                        if (elem.img.indexOf('/proxyimg') !== -1) elem.img = prox_prefix + account(elem.img);
                    }
        ...
        this.draw = function(items) {
            ...
            this.getEpisodes(items[0].season, function(episodes) {
                ...
                items.forEach(function(element, index) {
                    ...
                    _this8.contextMenu({ html: html, element: element, onFile: function onFile(call) {
                                ...
                                if (line && line.data && line.type == 'similar') {
                                    var cards = line.data.map(function(item) {
                                        ...
                                        if (item.img !== undefined) {
                                            ...
                                            if (item.img.indexOf('/proxyimg') !== -1) item.img = prox_prefix + account(item.img);
                                        }
                                        return item;
                                    });
