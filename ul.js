(function() {
    ...
    // возвращаем список как есть (чтобы видео всегда было играбельно)
    function filterMinQuality(qualityObj) {
        if (!qualityObj || typeof qualityObj !== 'object' || Array.isArray(qualityObj)) return qualityObj;
        var min = getMinQuality();
        if (!min) return qualityObj; // 0 = без фильтра
        var kept = {};
        Object.keys(qualityObj).forEach(function(k) {
            if (qualityScore(k) >= min) kept[k] = qualityObj[k];
        });
        if (!Object.keys(kept).length) return qualityObj; // фолбэк: ничего не подошло — не трогаем
        Object.keys(qualityObj).forEach(function(k) {
            if (!(k in kept)) delete qualityObj[k];
        });
        return qualityObj;
    }
    ...
    // ПАТЧ: следующий сервер по кругу (для авто-переключения при недоступности)
 ...
 function component(object) {
    ...
    this.initialize = function() {
        ...
        // ПАТЧ: показываем селектор "Сервер" сразу, чтобы можно было сменить
        // сервер даже пока идёт загрузка / если сервер завис
        filter.set('filter', [{
            title: 'Сервер',
            subtitle: (SERVER_CONFIG[connection_source] && SERVER_CONFIG[connection_source].getSubtitle) ? SERVER_CONFIG[connection_source].getSubtitle() : '',
    ...
    this.request = function(url) {
        ...
        function runRequest() {
            ...
            if (number_of_requests < 10) {
                ...
                network.timeout(15000); // ПАТЧ: явный таймаут на контент-запрос
        ...
        if (connection_source === 'skaz') {
            ...
            network.timeout(8000); // ПАТЧ: чтобы "будилка" не висела бесконечно
    ...
    this.similars = function(json) {
        var _this6 = this;
        this.stopWatchdog(); // ПАТЧ: получили ответ — гасим watchdog
        scroll.clear();
    ...
    this.draw = function(items) {
        var _this8 = this;
        this.stopWatchdog(); // ПАТЧ: есть данные — гасим watchdog
        ...
        // ПАТЧ: полностью переписан — больше не падает, кнопки рабочие,
        // авто-переключение на СЛЕДУЮЩИЙ сервер, возврат фокуса на экран
        this.noConnectToServer = function(er) {
            ...
            if (er && er.accsdb) {
                ...
                html.find('.online-empty__time').html(
                    Lampa.Lang.translate('lampac_does_not_answer_text').replace('{balanser}', SERVER_CONFIG[connection_source] ? SERVER_CONFIG[connection_source].label : (connection_source || '')) +
                    '<br>' + Lampa.Lang.translate('lampac_server_timeout')
                );
            }
            ...
            html.find('.change').text(Lampa.Lang.translate('lampac_change_server'));
            ...
            // ВАЖНО: возвращаем управление на экран, иначе кнопки недоступны
            Lampa.Controller.enable('content');
        ...
        this.destroy = function() {
            ...
            clearInterval(balanser_timer);
            clearTimeout(life_wait_timer);
            clearTimeout(load_timer); // ПАТЧ: гасим watchdog
        };
    }
    ...
    function startPlugin() {
        ...
        if (!window.plugin_iptvskaz_ready && !window.plugin_iptv_ready2) {
            ...
            // ПАТЧ: настраиваемый минимум качества (0 = без фильтра)
            Lampa.SettingsApi.addParam({
                component: 'iptvskaz',
                param: {
                    name: 'skaz_min_quality',
                    type: 'select',
                    values: {
                        '0': 'Без фильтра',
                        '720': 'от 720p',
                ...
                field: {
                    ...
                    description: 'Скрывать варианты ниже выбранного. Если ничего не подойдёт — покажет всё, что есть'
                }
            });
            Lampa.SettingsApi.addParam({
                component: 'iptvskaz',
    ...
    // ПАТЧ: getScript/play.js перенесены под guard, чтобы не грузиться повторно
    if (!window.onlyskaz_plugin) {
        startPlugin();
